...
export const run_command = tool(
    async ({ command, cwd }, config) => {
        ...
        if (sandboxid) {
            const sbx = await Sandbox.connect(sandboxid);
            const output = await sbx.commands.run(command, { cwd });
    ...
    , {
        name: "Run_command",
        description: "This is used to run a command. Use 'cwd' to specify the working directory (e.g., the cloned repo folder).",
        schema: z.object({
            command: z.string(),
...
export const run_command_background = tool(
    async ({ command, cwd }, config) => {
        ...
        if (sandboxid) {
            const sbx = await Sandbox.connect(sandboxid);
            const cmd = await sbx.commands.run(command, { background: true, cwd });
    ...
    },
    {
        ...
        description: "Run a command in background (e.g., dev servers). Use 'cwd' to specify the working directory.",
        schema: z.object({
            command: z.string(),
            cwd: z.string().optional()
        })
